...
// function Home() {
//   useEffect(() => {
//     // change status bar + body background when Home mounts
//     document.body.style.backgroundColor = "#0d2b26";
//     document.getElementById("theme-color-meta")
//       .setAttribute("content", "#0d2b26");

//     // reset back to white when leaving Home
//     return () => {
...
// The dark header/status-bar color used on this dashboard layout.
// Keep this in one place so it's easy to update later.
const STATUS_BAR_COLOR = '#0F3B39';
...
const DashboardLayout = ({ children }) => {
  ...
  // --------------------------------------------------------------------
  // Status bar / body background color.
  // DashboardLayout wraps every "app" page (dashboard, bible, sermons,
  // quiz, notifications, events, profile) with a dark header, so while
  // this layout is mounted we tint the status-bar strip + body to match
  // it. On unmount (e.g. navigating to /login, which does NOT use this
  // layout) it resets back to white automatically.
  // --------------------------------------------------------------------
  useEffect(() => {
    const themeColorMeta = document.getElementById('theme-color-meta');
    const previousBodyColor = document.body.style.backgroundColor;
    const previousMetaColor = themeColorMeta?.getAttribute('content');

    document.body.style.backgroundColor = STATUS_BAR_COLOR;
    themeColorMeta?.setAttribute('content', STATUS_BAR_COLOR);

    return () => {
      document.body.style.backgroundColor = previousBodyColor || '#ffffff';
      themeColorMeta?.setAttribute('content', previousMetaColor || '#ffffff');
    };
  }, []);
